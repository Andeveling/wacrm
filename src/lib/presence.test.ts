...
import { derivePresence, formatLastSeen, OFFLINE_AFTER_MS, presenceLabel, summarize } from './presence';
...
describe('derivePresence', () => {
  ...
  it('reads as offline when the heartbeat is stale', () => {
    expect(derivePresence('online', ago(OFFLINE_AFTER_MS + 1_000), NOW)).toBe('offline');
    // Stored 'away' goes stale to offline too (tab was closed while idle).
    expect(derivePresence('away', ago(OFFLINE_AFTER_MS + 1_000), NOW)).toBe('offline');
  });
  ...
  it('stays online exactly at the threshold and flips just past it', () => {
    ...
    expect(derivePresence('online', ago(OFFLINE_AFTER_MS + 1), NOW)).toBe('offline');
  });
});
...
describe('presenceLabel', () => {
  it('labels each state for the tooltip', () => {
    expect(presenceLabel('online', ago(1_000), NOW)).toBe('Online — active now');
    expect(presenceLabel('away', ago(1_000), NOW)).toBe('Away — idle');
    expect(presenceLabel('offline', ago(2 * 60 * 60_000), NOW)).toBe('Offline — last seen 2 hours ago');
  });
});

describe('summarize', () => {
  it('counts each status', () => {
    expect(summarize(['online', 'online', 'online', 'away', 'offline'])).toEqual({ online: 3, away: 1, offline: 1 });
  });
