...
describe('validateStepsForActivation', () => {
  it('rejects empty or missing step lists', () => {
    expect(validateStepsForActivation([])).toEqual([{ path: 'steps', message: 'active automations need at least one step' }]);
  ...
  it('flags every required field that is missing', () => {
    ...
    expect(issues.map((i) => i.path)).toEqual(['steps[0].text', 'steps[1].template_name', 'steps[2].tag_id']);
  });
  ...
  it('checks wait amount and unit boundaries', () => {
    ...
    expect(issues.map((i) => i.path)).toEqual(['steps[0].amount', 'steps[1].unit', 'steps[2].amount', 'steps[3].amount']);
  });
  ...
  it('validates webhook URLs', () => {
    ...
    const noUrl = validateStepsForActivation([{ step_type: 'send_webhook', step_config: {} }]);
    ...
    expect(wrongProtocol.map((i) => i.message)).toContain('webhook URL must use http or https');

    const garbage = validateStepsForActivation([{ step_type: 'send_webhook', step_config: { url: 'not a url' } }]);
    expect(garbage.map((i) => i.message)).toContain('webhook URL is not a valid URL');
  });
  ...
  it("validates assign_conversation only when mode is 'specific'", () => {
    ...
    const specificMissingAgent = validateStepsForActivation([{ step_type: 'assign_conversation', step_config: { mode: 'specific' } }]);
    expect(specificMissingAgent.map((i) => i.path)).toEqual(['steps[0].agent_id']);
  });

  it('flags create_deal when required fields are missing', () => {
    const issues = validateStepsForActivation([{ step_type: 'create_deal', step_config: {} }]);
    expect(issues.map((i) => i.path).sort()).toEqual(['steps[0].pipeline_id', 'steps[0].stage_id', 'steps[0].title']);
  });
  ...
  it('flags update_contact_field when field or value is missing', () => {
    ...
    expect(issues.map((i) => i.path)).toEqual(['steps[0].value', 'steps[1].field']);
  });
  ...
  it('recursively walks condition branches with stable dot-paths', () => {
    ...
    expect(issues.map((i) => i.path)).toEqual(['steps[0].yes.steps[0].tag_id', 'steps[0].no.steps[0].text']);
  });

  it('reports an issue for unknown step types', () => {
    const issues = validateStepsForActivation([{ step_type: 'do_a_barrel_roll', step_config: {} }]);
    expect(issues).toEqual([{ path: 'steps[0]', message: 'unknown step type: do_a_barrel_roll' }]);
  });

  it('flags condition subject/operand independently', () => {
    const issues = validateStepsForActivation([{ step_type: 'condition', step_config: {} }]);
    expect(issues.map((i) => i.path).sort()).toEqual(['steps[0].operand', 'steps[0].subject']);
  });
});
...
describe('validateTriggerForActivation', () => {
  ...
  it('rejects keyword_match with whitespace-only entries', () => {
    ...
    expect(issues.map((i) => i.message)).toContain('keywords cannot be empty strings');
  });
  ...
  it('accepts keyword_match with a missing match_type (defaults to contains)', () => {
    expect(validateTriggerForActivation('keyword_match', { keywords: ['hi'] })).toEqual([]);
  });

  it('requires schedule on time_based triggers', () => {
    expect(validateTriggerForActivation('time_based', {})).toEqual([{ path: 'trigger.schedule', message: 'schedule is required' }]);
    expect(validateTriggerForActivation('time_based', { schedule: '0 9 * * *' })).toEqual([]);
  });

  it('requires tag_id on tag_added triggers', () => {
    expect(validateTriggerForActivation('tag_added', {})).toEqual([{ path: 'trigger.tag_id', message: 'tag is required' }]);
    expect(validateTriggerForActivation('tag_added', { tag_id: 'tag-uuid' })).toEqual([]);
  });
  ...
  it('requires reply_ids on interactive_reply triggers', () => {
    ...
    expect(validateTriggerForActivation('interactive_reply', { reply_ids: ['yes', 'no'] })).toEqual([]);
    const empties = validateTriggerForActivation('interactive_reply', {
      reply_ids: ['yes', '  '],
    });
    expect(empties.map((i) => i.message)).toContain('reply ids cannot be empty strings');
  });
