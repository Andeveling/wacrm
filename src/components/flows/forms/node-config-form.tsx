...
export function NodeConfigForm({ node, allNodes, showAdvanced, onUpdateConfig }: NodeConfigFormProps) {
  ...
  switch (node.node_type) {
    ...
    case 'send_media':
      return (
        <SendMediaForm cfg={cfg as SendMediaCfg} allNodes={allNodes} currentKey={node.node_key} onUpdateConfig={onUpdateConfig} t={t} />
      );
    ...
    case 'collect_input':
      return (
        <>
          ...
          />
          <div>
            <label className="mb-1 block text-muted-foreground text-xs">{t('varKeyLabel')}</label>
            <Input
    ...
    case 'condition':
      return (
        <ConditionForm cfg={cfg as ConditionCfg} allNodes={allNodes} currentKey={node.node_key} onUpdateConfig={onUpdateConfig} t={t} />
      );

    case 'set_tag':
      return <SetTagForm cfg={cfg as SetTagCfg} allNodes={allNodes} currentKey={node.node_key} onUpdateConfig={onUpdateConfig} t={t} />;
...
function SendButtonsForm({
  ...
  const updateButton = (idx: number, patch: Partial<NonNullable<SendButtonsCfg['buttons']>[number]>) => {
    onUpdateConfig({
  ...
  const removeButton = (idx: number) => onUpdateConfig({ buttons: buttons.filter((_, i) => i !== idx) });

  return (
    <>
      <TextRow label={t('bodyText')} value={cfg.text ?? ''} onChange={(v) => onUpdateConfig({ text: v })} rows={3} />
      <TextRow label={t('footerText')} value={cfg.footer_text ?? ''} onChange={(v) => onUpdateConfig({ footer_text: v })} />
      <div>
...
function SendListForm({
  ...
  const updateSection = (sIdx: number, patch: Partial<NonNullable<SendListCfg['sections']>[number]>) => {
    onUpdateConfig({
  ...
  const removeSection = (sIdx: number) => onUpdateConfig({ sections: sections.filter((_, i) => i !== sIdx) });
  const updateRow = (sIdx: number, rIdx: number, patch: Partial<NonNullable<SendListCfg['sections']>[number]['rows'][number]>) => {
    onUpdateConfig({
  ...
  return (
    <>
      <TextRow label="Body text" value={cfg.text ?? ''} onChange={(v) => onUpdateConfig({ text: v })} rows={3} />
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <TextRow label={t('buttonLabel')} value={cfg.button_label ?? ''} onChange={(v) => onUpdateConfig({ button_label: v })} />
        <TextRow label={t('footerText')} value={cfg.footer_text ?? ''} onChange={(v) => onUpdateConfig({ footer_text: v })} />
      </div>

      <div className="mt-2">
        <label className="mb-2 block text-muted-foreground text-xs">{t('rowsHelp')}</label>
        {sections.map((section, sIdx) => (
          <div key={section.title || `section_${sIdx}`} className="mb-3 rounded-md border border-border bg-muted/40 p-3">
            ...
            {section.rows.map((row, rIdx) => (
              <div
                key={row.reply_id || `row_${sIdx}_${rIdx}`}
                className={cn(
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        <div>
          <label className="mb-1 block text-muted-foreground text-xs">{t('ifLabel')}</label>
          <Select value={subject} onValueChange={(v) => onUpdateConfig({ subject: v as ConditionCfg['subject'] })}>
        ...
        <div className="md:col-span-2">
          ...
          {subject === 'tag' && tags.length > 0 ? (
            <Select value={cfg.subject_key ?? ''} onValueChange={(v) => onUpdateConfig({ subject_key: v })}>
              <SelectTrigger className="bg-muted">
      ...
      <div className={cn('grid grid-cols-1 gap-3', showValue ? 'md:grid-cols-2' : '')}>
        <div>
          <label className="mb-1 block text-muted-foreground text-xs">{t('operatorLabel')}</label>
          <Select value={operator} onValueChange={(v) => onUpdateConfig({ operator: v as ConditionCfg['operator'] })}>
            <SelectTrigger className="bg-muted">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        {showValue && (
          <div>
            <label className="mb-1 block text-muted-foreground text-xs">{t('valueLabel')}</label>
            <Input value={cfg.value ?? ''} onChange={(e) => onUpdateConfig({ value: e.target.value })} className="bg-muted" />
          </div>
        )}
      </div>
...
function SetTagForm({
  ...
  return (
    <>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
        <div>
          <label className="mb-1 block text-muted-foreground text-xs">{t('actionLabel')}</label>
          <Select value={cfg.mode ?? 'add'} onValueChange={(v) => onUpdateConfig({ mode: v as SetTagCfg['mode'] })}>
            <SelectTrigger className="bg-muted">
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <div>
          <label className="mb-1 block text-muted-foreground text-xs">{t('tagLabel')}</label>
          {tags.length > 0 ? (
...
function SendMediaForm({
  ...
  return (
    <>
      <div>
        <label className="mb-1 block text-muted-foreground text-xs">{t('mediaTypeLabel')}</label>
        <Select
          value={mediaType}
          onValueChange={(v) => {
      ...
      <div>
        <label className="mb-1 block text-muted-foreground text-xs">{t('fileLabel')}</label>
        {cfg.media_url ? (
          ...
          <button
            ...
            disabled={uploading}
            className="flex w-full items-center justify-center gap-2 rounded-md border border-border border-dashed bg-card px-3 py-4 text-muted-foreground text-xs transition-colors hover:border-border hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-60"
          >
            {uploading ? (
              <>
      ...
      <TextRow label={t('captionLabel')} value={cfg.caption ?? ''} onChange={(v) => onUpdateConfig({ caption: v })} rows={2} />

      {isDocument && (
        <div>
          <label className="mb-1 block text-muted-foreground text-xs">{t('filenameLabel')}</label>
          <Input
            value={cfg.filename ?? ''}
