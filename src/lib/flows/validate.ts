...
export function validateFlowForActivation(flow: FlowInput, nodes: NodeInput[]): ValidationIssue[] {
...
function validateTrigger(trigger_type: FlowInput['trigger_type'], trigger_config: Record<string, unknown>): ValidationIssue[] {
  const issues: ValidationIssue[] = [];

  if (trigger_type === 'keyword') {
    const keywords = Array.isArray(trigger_config.keywords) ? (trigger_config.keywords as unknown[]) : null;
...
function validateNode(node: NodeInput, knownKeys: Set<string>): ValidationIssue[] {
  ...
  switch (node.node_type) {
    ...
    case 'send_media': {
      ...
      if (!cfg.media_type || !['image', 'video', 'document'].includes(cfg.media_type)) {
        issues.push({
          severity: 'error',
          scope: 'node',
          node_key: node.node_key,
    ...
    case 'send_list': {
      ...
      sections.forEach((section, si) => {
        ...
        rows.forEach((row, ri) => {
          ...
          } else if (row.title.length > INTERACTIVE_LIMITS.listRowTitleMaxLength) {
            issues.push({
              severity: 'error',
              scope: 'node',
              node_key: node.node_key,
    ...
    case 'condition': {
      ...
      } else if ((cfg.operator === 'equals' || cfg.operator === 'contains') && (cfg.value === undefined || cfg.value === '')) {
        issues.push({
          severity: 'warning',
          scope: 'node',
          node_key: node.node_key,
...
export function reachableFromEntry(entryKey: string, nodes: NodeInput[]): Set<string> {
