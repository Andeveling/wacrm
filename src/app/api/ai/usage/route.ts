...
export async function GET(request: Request) {
  try {
    ...
    // to a single day.
    const days = Number.isFinite(rawDays) && rawDays >= 1 ? Math.min(90, Math.floor(rawDays)) : DEFAULT_WINDOW_DAYS;
    ...
    const modelMap = new Map<string, { model: string; provider: string; calls: number; tokens: number }>();
    ...
    for (const r of rows) {
      ...
      const m = modelMap.get(mk) ?? { model: r.model, provider: r.provider, calls: 0, tokens: 0 };
      m.calls += 1;
