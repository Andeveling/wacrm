...
import { applyEdgeConnection, deriveCanvasEdges, outgoingSlots, unlinkNodeReferences } from './edges';
...
describe('deriveCanvasEdges — single-outgoing node types', () => {
  ...
  it('derives a `next` edge from send_media, set_tag, collect_input, start', () => {
    ...
    expect(edges.map((e) => `${e.source}->${e.target}`)).toEqual(['s->m', 'm->t', 't->ci', 'ci->e']);
  });
      ...
      nodes(
        {
          ...
          config: {
            ...
            sections: [
              {
                title: 'Recent',
                rows: [{ reply_id: 'o1', title: 'Order 1', next_node_key: 'a' }],
              },
              {
                title: 'Older',
                rows: [{ reply_id: 'o2', title: 'Order 2', next_node_key: 'b' }],
              },
            ],
          },
        },
...
describe('deriveCanvasEdges — terminal nodes', () => {
  it('emits no outgoing edges from handoff / end', () => {
    const edges = deriveCanvasEdges(
      nodes({ node_key: 'h', node_type: 'handoff', config: { note: 'x' } }, { node_key: 'e', node_type: 'end', config: {} })
    );
    expect(edges).toEqual([]);
  });
});
...
describe('outgoingSlots', () => {
  it("returns a single 'next' slot for the auto-advancing types", () => {
    ...
    expect(each({ node_key: 'x', node_type: 'start', config: { next_node_key: 'y' } })).toEqual(['next']);
    expect(each({ node_key: 'x', node_type: 'send_message', config: {} })).toEqual(['next']);
    expect(each({ node_key: 'x', node_type: 'send_media', config: {} })).toEqual(['next']);
    expect(each({ node_key: 'x', node_type: 'collect_input', config: {} })).toEqual(['next']);
    expect(each({ node_key: 'x', node_type: 'set_tag', config: {} })).toEqual(['next']);
  });
  ...
  it('terminal nodes (handoff / end) have no outgoing slots', () => {
    expect(outgoingSlots({ node_key: 'h', node_type: 'handoff', config: {} })).toEqual([]);
    expect(outgoingSlots({ node_key: 'e', node_type: 'end', config: {} })).toEqual([]);
  });
});
...
describe('applyEdgeConnection', () => {
  ...
  it('returns null for terminal nodes (no outgoing)', () => {
    expect(applyEdgeConnection({ node_key: 'h', node_type: 'handoff', config: {} }, 'next', 'x')).toBeNull();
    expect(applyEdgeConnection({ node_key: 'e', node_type: 'end', config: {} }, 'next', 'x')).toBeNull();
  });
});
