...
import { deleteAccountMedia, MEDIA_MAX_BYTES_BY_KIND, uploadAccountMedia } from '@/lib/storage/upload-media';
...
export function MessageComposer({
  ...
  const [interactivePayload, setInteractivePayload] = useState<InteractiveMessagePayload>(blankButtonsPayload);
  ...
  const openInteractiveBuilder = useCallback((seed?: InteractiveMessagePayload) => {
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      if (file.size > max) {
        toast.error(`File is ${(file.size / 1024 / 1024).toFixed(1)} MB — ${kind} limit is ${Math.round(max / 1024 / 1024)} MB.`);
        return;
      }
      setBusy(true);
      try {
    ...
    <div className="border-border border-t bg-card p-3">
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-amber-400 text-xs">{t('sessionExpiredHint')}</p>
          <Button
            variant="ghost"
            size="sm"
            disabled={readOnly}
            className="h-7 text-amber-400 text-xs hover:text-amber-300"
      ...
      {draft ? (
        ...
        <div className="flex items-center gap-3 rounded-xl border border-border bg-muted px-4 py-2.5">
          ...
          <button
            ...
            className="rounded-md px-2 py-1 text-muted-foreground text-xs hover:bg-card hover:text-foreground"
          >
            {t('cancel')}
          </button>
          <Button
        ...
        <div className="flex items-end gap-2">
          ...
          <textarea
            ...
            title={readOnly ? (archived ? t('archivedTitle') : t('readOnlyTitle')) : undefined}
            className={cn(
              'flex-1 resize-none rounded-xl border border-border bg-muted px-4 py-2.5 text-foreground text-sm placeholder-muted-foreground outline-none transition-colors focus:border-primary/50',
      ...
      {!draft && !recording && <p className="mt-1 pl-[5.5rem] text-[10px] text-muted-foreground">{t('draftHint')}</p>}
      ...
      <Dialog open={interactiveOpen} onOpenChange={setInteractiveOpen}>
        <DialogContent className="sm:max-w-2xl">
          ...
          <div className="max-h-[70vh] overflow-y-auto">
            <InteractiveBuilder value={interactivePayload} onChange={setInteractivePayload} />
          </div>
          <DialogFooter>
            <Button variant="outline" disabled={savingQuickReply} onClick={saveAsQuickReply}>
              {savingQuickReply ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <Zap className="mr-1 h-4 w-4" />}
              {t('saveAsQuickReply')}
            </Button>
      ...
      {/* Quick-reply picker. */}
      <QuickReplyPicker open={quickReplyOpen} onOpenChange={setQuickReplyOpen} onPick={handlePickQuickReply} />
    </div>
  );
}
      ...
      <div className="flex items-start gap-3">
        <div className="min-w-0 flex-1">
          {draft.kind === 'image' && (
            ...
            <img src={draft.mediaUrl} alt={draft.filename} className="max-h-40 rounded-lg object-cover" />
          )}
          {/* biome-ignore lint/a11y/useMediaCaption: draft preview of user-uploaded media, no captions source */}
          {draft.kind === 'video' && <video src={draft.mediaUrl} controls className="max-h-40 rounded-lg" />}
          {/* biome-ignore lint/a11y/useMediaCaption: draft preview of user-uploaded media, no captions source */}
          {draft.kind === 'audio' && <audio src={draft.mediaUrl} controls className="w-full" />}
          {draft.kind === 'document' && (
      ...
      <div className="mt-2 flex items-end gap-2">
        {draft.kind !== 'audio' && (
          <input
            ...
            className="flex-1 rounded-xl border border-border bg-muted px-4 py-2.5 text-foreground text-sm placeholder-muted-foreground outline-none transition-colors focus:border-primary/50"
          />
        )}
        <GatedButton
          size="sm"
          ...
          disabled={busy}
          onClick={onSend}
          className={cn('h-9 w-9 shrink-0 bg-primary p-0 hover:bg-primary/90 disabled:opacity-40', draft.kind === 'audio' && 'ml-auto')}
        >
          <Send className="h-4 w-4" />
        </GatedButton>
      </div>
