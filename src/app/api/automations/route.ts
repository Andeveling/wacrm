...
import { type BuilderStepInput, insertSteps } from '@/lib/automations/steps-tree';
...
export async function POST(request: Request) {
  ...
  if (is_active) {
    ...
    if (issues.length > 0) {
      return NextResponse.json({ error: 'Cannot activate automation with invalid configuration', issues }, { status: 400 });
    }
  }
