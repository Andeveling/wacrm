import { getTagChainDepth, MAX_TAG_CHAIN_DEPTH } from '@/lib/contacts/tag-chain';
...
export async function resumePendingExecution(pending: {
  ...
  const db = supabaseAdmin();
  const { data: automation, error } = await db.from('automations').select('*').eq('id', pending.automation_id).single();
    ...
    case 'update_contact_field': {
      ...
      if (cfg.field.startsWith('custom:')) {
        ...
        await db
          ...
          .upsert({ contact_id: args.contactId, custom_field_id: customFieldId, value }, { onConflict: 'contact_id,custom_field_id' });
