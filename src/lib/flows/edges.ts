    ...
    switch (node.node_type) {
      ...
      case 'send_buttons': {
        ...
        for (const btn of buttons) {
          const replyId = typeof btn.reply_id === 'string' ? btn.reply_id : null;
          const next = typeof btn.next_node_key === 'string' ? btn.next_node_key : null;
      ...
      case 'send_list': {
        ...
        for (const section of sections) {
          ...
          for (const row of rows) {
            ...
            const next = typeof row.next_node_key === 'string' ? row.next_node_key : null;
 ...
 * unchanged.
 */
export function applyEdgeConnection(node: BuilderNode, sourceHandle: string, targetKey: string): Record<string, unknown> | null {
  switch (node.node_type) {
    case 'start':
    case 'send_message':
    case 'send_media':
    ...
    case 'send_buttons': {
      ...
      return {
        buttons: buttons.map((b) => (b.reply_id === replyId ? { ...b, next_node_key: targetKey } : b)),
      };
    }
    ...
    case 'send_list': {
      ...
      const next = sections.map((s) => {
        const rows = Array.isArray(s.rows) ? (s.rows as Array<Record<string, unknown>>) : [];
        return {
          ...s,
          rows: rows.map((r) => {
 ...
 */
export function unlinkNodeReferences(nodes: BuilderNode[], deletedKey: string): BuilderNode[] {
  return nodes.map((n) => {
...
function patchedConfigWithoutKey(node: BuilderNode, deletedKey: string): Record<string, unknown> | null {
  const cfg = node.config;
  switch (node.node_type) {
    case 'start':
    case 'send_message':
    ...
    case 'send_buttons': {
      ...
      return {
        ...cfg,
        buttons: buttons.map((b) => (b.next_node_key === deletedKey ? { ...b, next_node_key: '' } : b)),
      };
    }
