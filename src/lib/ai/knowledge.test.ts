...
function makeDb() {
  ...
  const db = {
    rpc: (name: string) => {
      ...
      if (name === 'match_ai_knowledge_semantic') return Promise.resolve({ data: state.semantic, error: null });
      if (name === 'match_ai_knowledge_fts') return Promise.resolve({ data: state.fts, error: null });
...
beforeEach(() => {
  ...
  h.embedTexts.mockImplementation(async (_key: string, inputs: string[]) => inputs.map((_, i) => [i, i]));
});
...
describe('ingestDocument', () => {
  ...
  it('still stores lexical chunks when embedding fails, then rethrows', async () => {
    ...
    await expect(ingestDocument(db, 'acct', { embeddingsApiKey: 'sk-x' }, 'doc-1', 'hello world')).rejects.toThrow('rate limited');
