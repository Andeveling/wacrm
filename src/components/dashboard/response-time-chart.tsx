...
export function ResponseTimeChart({ data, loading, thresholdMinutes = 5 }: ResponseTimeChartProps) {
    ...
    <section className="rounded-xl border border-border bg-card">
          ...
          {data && (data.thisWeekAvg != null || data.lastWeekAvg != null) && (
            <div>
              <div className="text-muted-foreground">
                {t('thisWeek')} <span className="font-medium text-foreground tabular-nums">{fmt(data.thisWeekAvg)}</span>
              </div>
              <div className="text-muted-foreground">
                {t('lastWeek')} <span className="tabular-nums">{fmt(data.lastWeekAvg)}</span>
              </div>
            </div>
          )}
        </div>
      ...
      <div className="p-5">
        {loading || !data ? (
          ...
          <EmptyState icon={Clock} title={t('noReplies')} hint={t('noRepliesHint')} />
        ) : (
          <BarChart
            data={chartData}
            index="day"
