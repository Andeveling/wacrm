...
export async function PATCH(request: Request) {
  try {
    ...
    const limit = checkRateLimit(`admin:rename:${ctx.userId}`, RATE_LIMITS.adminAction);
    ...
    const { data, error } = await ctx.supabase.from('accounts').update({ name }).eq('id', ctx.accountId).select('id, name').single();
