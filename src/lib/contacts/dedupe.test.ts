...
import { dedupeByPhone, findExistingContact, isExactMatch, isUniqueViolation, normalizeKey } from './dedupe';
...
describe('isExactMatch', () => {
  it('treats different formatting of the same digits as exact', () => {
    expect(isExactMatch({ id: '1', phone: '+1 555-123-4567' }, '15551234567')).toBe(true);
  });

  it('is false for a trunk-variant (fuzzy) match', () => {
    // last-8 match but not the same full number
    expect(isExactMatch({ id: '1', phone: '37063949836' }, '370063949836')).toBe(false);
  });
});
...
describe('dedupeByPhone', () => {
  ...
  it('drops rows with no digits', () => {
    const { unique, duplicates } = dedupeByPhone([{ phone: '   ' }, { phone: '+1 555-3333' }]);
