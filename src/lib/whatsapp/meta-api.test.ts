...
import { INTERACTIVE_LIMITS, sendInteractiveButtons, sendInteractiveList } from './meta-api';
...
describe('sendInteractiveButtons — validation', () => {
  ...
  it('rejects an empty buttons array', async () => {
    await expect(sendInteractiveButtons({ ...BASE_ARGS, buttons: [] })).rejects.toThrow(/1-3 buttons/);
  });
  ...
  it('rejects a button title longer than 20 chars (Meta cap)', async () => {
    await expect(
      sendInteractiveButtons({
        ...BASE_ARGS,
        buttons: [{ id: 'a', title: 'x'.repeat(INTERACTIVE_LIMITS.buttonTitleMaxLength + 1) }],
      })
  ...
  it('sends the right payload shape when all inputs are valid', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (url: string, init: RequestInit) => {
        ...
        return new Response(JSON.stringify({ messages: [{ id: 'wamid.PASS' }] }), { status: 200 });
      })
    );
  ...
  it('rejects duplicate row ids across sections', async () => {
    await expect(
      sendInteractiveList({
        ...BASE_ARGS,
        buttonLabel: 'Open',
        sections: [{ rows: [{ id: 'dupe', title: 'First' }] }, { rows: [{ id: 'dupe', title: 'Second' }] }],
      })
