...
import { Briefcase, Inbox, MessageSquare, Radio, UserPlus, Zap } from 'lucide-react';
...
export function ActivityFeed({ items, loading }: ActivityFeedProps) {
  ...
  const isSizeUseful = (_size: PageSize, i: number) => i === 0 || totalLoaded > PAGE_SIZES[i - 1];
  ...
  return (
    <section className="rounded-xl border border-border bg-card">
      <header className="flex items-center justify-between border-border border-b px-5 py-4">
        ...
        <Link href="/inbox" className="font-medium text-primary text-xs hover:text-primary/80">
          {t('viewAll')}
        </Link>
      </header>
      ...
      {loading || !items ? (
        <div className="space-y-2 p-5">
          <Skeleton key="activity-skeleton-0" className="h-10 w-full" />
          <Skeleton key="activity-skeleton-1" className="h-10 w-full" />
          <Skeleton key="activity-skeleton-2" className="h-10 w-full" />
          <Skeleton key="activity-skeleton-3" className="h-10 w-full" />
          <Skeleton key="activity-skeleton-4" className="h-10 w-full" />
        </div>
      ) : items.length === 0 ? (
        <div className="p-5">
        ...
        <>
            ...
            {visible.map((it, i) => {
              ...
              const row = (
                <div className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <span className="min-w-0 flex-1 truncate text-foreground text-sm">{it.text}</span>
                  <span className="flex-shrink-0 text-muted-foreground text-xs tabular-nums">{relativeTime(it.at, t)}</span>
                </div>
              );
              return (
          ...
          <footer className="flex items-center justify-between border-border border-t px-5 py-3 text-xs">
