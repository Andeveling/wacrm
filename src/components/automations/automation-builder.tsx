...
import { blankButtonsPayload, blankListPayload, InteractiveBuilder } from '@/components/interactive/interactive-builder';
...
function ResourcesProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <ResourcesContext.Provider value={{ tags, members, templates, customFields, pipelines, stages }}>{children}</ResourcesContext.Provider>
  );
}
...
function SendTemplateFields({
  ...
  const hasMatch = templates.some((t) => toValue(t.name, t.language ?? 'en_US') === current);
  ...
  return (
    <FieldBlock label={t('templates.templateLabel')}>
      ...
      >
        ...
        {current && !hasMatch && (
          <option value={current}>{t('templates.unknown', { name: templateName, lang: language || t('templates.unknownLang') })}</option>
        )}
      </select>
    </FieldBlock>
  );
  ...
  return (
    <div className="fixed inset-0 flex flex-col bg-background">
      {/* Top bar. At sub-sm widths the "Active" label is hidden and the
          ...
          gets maximum width. */}
      <header className="flex flex-shrink-0 items-center gap-2 border-border border-b bg-card/80 px-3 py-3 sm:gap-3 sm:px-4">
        <button
          type="button"
        ...
        <input
          ...
          className="min-w-0 flex-1 rounded-md bg-transparent px-2 py-1 font-semibold text-foreground text-sm placeholder:text-muted-foreground focus:bg-muted focus:outline-none sm:text-base"
        />
        <div className="flex items-center gap-2 text-muted-foreground text-xs">
          <span className="hidden sm:inline">{t('active')}</span>
          <Switch checked={state.is_active} onCheckedChange={(v) => patchTop('is_active', !!v)} aria-label={t('activeAria')} />
        </div>
      ...
      <div className="rounded-lg border border-border border-l-4 border-l-blue-500 bg-card shadow-lg">
        <button type="button" onClick={() => setOpen((v) => !v)} className="flex w-full items-center gap-3 px-4 py-3 text-left">
          ...
          <div className="min-w-0 flex-1">
            <div className="text-[11px] text-blue-300 uppercase tracking-wide">{t('trigger')}</div>
            <div className="truncate font-medium text-foreground text-sm">{t(`triggers.${type}.label`)}</div>
          </div>
            ...
            <div>
              <label className="mb-1 block font-medium text-muted-foreground text-xs">
                ...
                <select
                  ...
                  className="mt-1 w-full rounded-md border border-border bg-muted px-2 py-1.5 text-foreground text-sm focus:border-primary focus:outline-none"
                >
              ...
              <p className="mt-1 text-[11px] text-muted-foreground">{t(`triggers.${type}.hint`)}</p>
            </div>
            {type === 'keyword_match' && (
              <KeywordMatchConfig config={config as unknown as KeywordMatchTriggerConfig} onChange={onConfigChange} t={t} />
            )}
            {type === 'interactive_reply' && <InteractiveReplyConfig config={config} onChange={onConfigChange} t={t} />}
            {type === 'tag_added' && (
              <label className="mb-1 block font-medium text-muted-foreground text-xs">
                Tag
                <TagSelect value={(config.tag_id as string) ?? ''} onChange={(v) => onConfigChange({ ...config, tag_id: v })} t={t} />
              </label>
            )}
            {type === 'time_based' && (
              <div>
      ...
      <div>
        <label className="mb-1 block font-medium text-muted-foreground text-xs">
          ...
          <select
            ...
            className="mt-1 w-full rounded-md border border-border bg-muted px-2 py-1.5 text-foreground text-sm focus:outline-none"
          >
...
function StepRenderer({
  ...
  return (
    <>
        ...
        <div className={cn('rounded-lg border border-border border-l-4 bg-card shadow-lg', meta.border)}>
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              <div className="truncate font-medium text-foreground text-sm">{t(`steps.${meta.label}`)}</div>
      ...
      {!isCondition && <AddButton onPick={(t) => props.addStepAt(parentScope, index + 1, t)} />}
    </>
  );
}
...
function BranchColumn({ label, color, children }: { label: string; color: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col items-center">
      <div className={cn('mb-2 font-semibold text-[11px] uppercase', color)}>{label}</div>
      {children}
    </div>
  );
}
...
function AddButton({ onPick }: { onPick: (t: AutomationStepType) => void }) {
  ...
  return (
    <div className="relative flex flex-col items-center">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger
          className="flex h-8 w-8 items-center justify-center rounded-full border-2 border-border border-dashed bg-background text-muted-foreground transition-colors hover:border-primary hover:bg-primary/10 hover:text-primary data-[popup-open]:border-primary data-[popup-open]:bg-primary/20 data-[popup-open]:text-primary"
...
function StepEditor({ step, onChange }: { step: BuilderStep; onChange: (s: BuilderStep) => void }) {
  ...
  switch (step.step_type) {
    ...
    case 'send_list':
      ...
      return (
        <InteractiveBuilder value={asInteractive(cfg)} onChange={(payload) => onChange({ ...step, step_config: toStepConfig(payload) })} />
      );
    case 'send_template':
      return (
        <SendTemplateFields
    ...
    case 'remove_tag':
      return (
        <FieldBlock label={t('config.tagLabel')}>
          <TagSelect value={(cfg.tag_id as string) ?? ''} onChange={(v) => set({ tag_id: v })} t={t} />
        </FieldBlock>
      );
    case 'assign_conversation':
      return (
        <>
          <FieldBlock label={t('config.modeLabel')}>
            <select
              ...
              className="w-full rounded-md border border-border bg-muted px-2 py-1.5 text-foreground text-sm"
            >
          ...
          {cfg.mode === 'specific' && (
            <FieldBlock label={t('config.agentLabel')}>
              <AgentSelect value={(cfg.agent_id as string) ?? ''} onChange={(v) => set({ agent_id: v })} t={t} />
            </FieldBlock>
          )}
        </>
      );
      ...
      return (
        <>
          <FieldBlock label={t('config.fieldLabel')}>
            <ContactFieldSelect value={(cfg.field as string) ?? 'name'} onChange={(v) => set({ field: v })} t={t} />
          </FieldBlock>
      ...
      return (
        <>
          <FieldBlock label={t('config.urlLabel')}>
            <Input value={(cfg.url as string) ?? ''} onChange={(e) => set({ url: e.target.value })} className="bg-muted text-foreground" />
          </FieldBlock>
          <FieldBlock label={t('config.bodyTemplateLabel')}>
            <Textarea
              value={(cfg.body_template as string) ?? ''}
              onChange={(e) => set({ body_template: e.target.value })}
              className="min-h-20 bg-muted font-mono text-foreground text-xs"
            />
          </FieldBlock>
        </>
      );
...
function FieldBlock({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <label className="mb-2 block font-medium text-muted-foreground text-xs last:mb-0">
