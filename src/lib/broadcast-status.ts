...
export function getBroadcastStatus(status: string): StatusDisplay {
  return broadcastStatusConfig[status as BroadcastStatus] ?? broadcastStatusConfig.draft;
}

export function getRecipientStatus(status: string): StatusDisplay {
  return recipientStatusConfig[status as RecipientStatus] ?? recipientStatusConfig.pending;
}
