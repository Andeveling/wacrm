...
import { Clock, Copy, FileText, Loader2, MessageCircle, MoreVertical, Pencil, PhoneCall, Plus, Trash2, Users, Zap } from 'lucide-react';
...
const TEMPLATE_ORDER: TemplateSlug[] = ['welcome_message', 'out_of_office', 'lead_qualifier', 'follow_up_reminder'];
...
export default function AutomationsPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const { data, error: fetchErr } = await supabase.from('automations').select('*').order('created_at', { ascending: false });
  ...
  return (
    <div className="space-y-6">
      ...
      {showTemplates && (
        <section>
          <h2 className="mb-3 font-semibold text-muted-foreground text-sm">{t('templatesTitle')}</h2>
      ...
      {automations.length === 0 ? (
        <div className="flex h-48 flex-col items-center justify-center rounded-xl border border-border border-dashed bg-card/40">
          ...
          <p className="mt-3 font-medium text-foreground text-sm">{t('emptyTitle')}</p>
      ...
      <div className="flex items-center gap-4 p-4">
        ...
        <button type="button" onClick={onEdit} className="min-w-0 flex-1 text-left">
          <div className="flex items-center gap-2">
            <span className="truncate font-semibold text-foreground text-sm">{automation.name}</span>
            {automation.is_active && (
              <span role="img" aria-label="active" className="relative flex h-2 w-2">
                <span className="absolute inline-flex h-full w-full animate-ping rounded-full bg-primary opacity-75" />
                <span className="relative inline-flex h-2 w-2 rounded-full bg-primary" />
              </span>
            )}
          </div>
          {automation.description && <p className="mt-0.5 truncate text-muted-foreground text-xs">{automation.description}</p>}
          <div className="mt-2 flex flex-wrap items-center gap-2 text-muted-foreground text-xs">
            <span className={cn('inline-flex items-center rounded-full border px-2 py-0.5 font-medium text-[11px]', meta.pillClass)}>
              {meta.label}
            </span>
