...
export async function loadAiConfig(
  ...
  const { data, error } = await db.from('ai_configs').select(CONFIG_COLUMNS).eq('account_id', accountId).maybeSingle();
 ...
 */
export async function loadEmbeddingsKey(db: SupabaseClient, accountId: string): Promise<{ key: string | null; corrupt: boolean }> {
  const { data, error } = await db.from('ai_configs').select('embeddings_api_key').eq('account_id', accountId).maybeSingle();
  if (error || !data?.embeddings_api_key) return { key: null, corrupt: false };
  try {
    return { key: decrypt(data.embeddings_api_key), corrupt: false };
  } catch {
    console.error(`[ai config] embeddings key for account ${accountId} could not be decrypted — check ENCRYPTION_KEY.`);
