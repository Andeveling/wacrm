...
export async function replaceSteps(automationId: string, input: BuilderStepInput[]): Promise<string | null> {
  const admin = supabaseAdmin();
  const { error: delErr } = await admin.from('automation_steps').delete().eq('automation_id', automationId);
  if (delErr) return delErr.message;
  return insertSteps(automationId, input);
}

export async function insertSteps(automationId: string, input: BuilderStepInput[]): Promise<string | null> {
...
export async function loadStepsTree(automationId: string): Promise<BuilderStepNode[]> {
  ...
  for (const row of rows) {
    const node = byId.get(row.id) as BuilderStepNode;
