...
import { CircleAlert, Loader2, Mail, Trash2, Upload } from 'lucide-react';
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pendingAvatar) {
        ...
        const { error: uploadError } = await supabase.storage.from('avatars').upload(path, pendingAvatar, {
          cacheControl: '3600',
          upsert: true,
      ...
      toast.success(emailSent ? t('profileSavedEmailCheck') : t('profileSaved'));
    } catch (err) {
          ...
          <CardContent className="space-y-6">
            ...
            <div className="flex flex-wrap items-center gap-5">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <p className="w-full text-muted-foreground text-xs">{t('photoHint')}</p>
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              {emailChangePending && (
                <p className="flex items-start gap-2 rounded-md border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-amber-700 text-xs dark:text-amber-300">
            ...
            <div className="rounded-lg border border-border bg-muted p-4">
              <p className="mb-3 font-semibold text-muted-foreground text-xs uppercase tracking-wider">{t('accountDetails')}</p>
              <dl className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2">
                <div>
                  <dt className="text-muted-foreground">{t('role')}</dt>
                  <dd className="mt-0.5 font-mono text-foreground">{profile?.role ?? 'user'}</dd>
                </div>
                <div>
                ...
                <div className="sm:col-span-2">
                  ...
                  <dd className="mt-0.5 break-all font-mono text-muted-foreground text-xs">{user?.id ?? '—'}</dd>
                </div>
              </dl>
            </div>
