...
import { type AiConfig, AiError } from './types';
...
describe('generateReply — OpenAI', () => {
  ...
  it('maps a 401 to an invalid_key AiError', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(errResponse(401, { error: { message: 'Incorrect API key' } })));
...
describe('generateReply — Anthropic', () => {
  ...
  it('detects handoff in the model output', async () => {
    vi.stubGlobal('fetch', vi.fn().mockResolvedValue(okResponse({ content: [{ type: 'text', text: '[[HANDOFF]]' }] })));
    const res = await generateReply({
