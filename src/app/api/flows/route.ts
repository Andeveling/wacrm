...
async function requireUser(): Promise<
  { ok: true; userId: string; supabase: Awaited<ReturnType<typeof createClient>> } | { ok: false; status: number; body: { error: string } }
> {
...
export async function POST(request: Request) {
  ...
  const { data: profile } = await supabase.from('profiles').select('account_id').eq('user_id', userId).single();
