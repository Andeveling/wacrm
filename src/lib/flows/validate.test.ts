...
describe('validateFlowForActivation — flow-level', () => {
  it('flags empty name', () => {
    expect(validateFlowForActivation({ ...validFlow, name: '' }, validNodes)).toEqual(
      expect.arrayContaining([expect.objectContaining({ scope: 'flow', field: 'name' })])
    );
  });

  it('flags whitespace-only name', () => {
    const issues = validateFlowForActivation({ ...validFlow, name: '   ' }, validNodes);
    expect(issues.some((i) => i.field === 'name')).toBe(true);
  });

  it('flags missing entry_node_id', () => {
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: null }, validNodes);
    expect(issues.some((i) => i.field === 'entry_node_id')).toBe(true);
  });

  it("flags entry_node_id that doesn't exist in nodes", () => {
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 'ghost' }, validNodes);
    expect(issues.some((i) => i.field === 'entry_node_id' && i.message.includes('"ghost"'))).toBe(true);
  });

  it('flags empty node list', () => {
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: null }, []);
    expect(issues.some((i) => i.message.includes('at least one node'))).toBe(true);
  });
  ...
  it('flags duplicate node_key', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 'a' }, dupes);
    expect(issues.some((i) => i.message.includes('Duplicate node_key') && i.node_key === 'a')).toBe(true);
  });
});
...
describe('validateFlowForActivation — trigger', () => {
  it('flags keyword trigger with no keywords', () => {
    ...
    expect(issues.some((i) => i.scope === 'trigger' && i.message.includes('at least one keyword'))).toBe(true);
  });

  it('flags keyword trigger missing keywords field entirely', () => {
    const issues = validateFlowForActivation({ ...validFlow, trigger_config: {} }, validNodes);
  ...
  it('warns when keywords contain blanks', () => {
    ...
    expect(issues.some((i) => i.scope === 'trigger' && i.severity === 'warning' && i.message.includes('blank'))).toBe(true);
  });
...
describe('validateFlowForActivation — nodes', () => {
  it('flags send_buttons without text', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'b' && i.field === 'text')).toBe(true);
  });
  ...
  it('flags send_buttons with zero buttons', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'b' && i.field === 'buttons' && i.message.includes('at least one'))).toBe(true);
  });
  ...
  it('flags send_buttons with more than 3 buttons (Meta limit)', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'b' && i.field === 'buttons' && i.message.includes('at most 3'))).toBe(true);
  });
  ...
  it('flags button title over 20 chars', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'b' && i.field === 'buttons.0.title' && i.message.includes('over 20'))).toBe(true);
  });
  ...
  it('flags button pointing at non-existent next node', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.field === 'buttons.0.next_node_key' && i.message.includes('ghost'))).toBe(true);
  });
  ...
  it('flags duplicate button reply_ids', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.message.includes('Duplicate button reply id'))).toBe(true);
  });
  ...
  it('flags send_list with more than 10 rows total', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'l' && i.field === 'sections' && i.message.includes('at most 10'))).toBe(true);
  });
  ...
  it('flags list row title over 24 chars', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.message.includes('exceeds 24 chars'))).toBe(true);
  });
  ...
  it('warns about unreachable nodes', () => {
    ...
    const issues = validateFlowForActivation({ ...validFlow, entry_node_id: 's' }, nodes);
    expect(issues.some((i) => i.node_key === 'orphan' && i.severity === 'warning' && i.message.includes('unreachable'))).toBe(true);
  });
...
describe('validateFlowForActivation — send_media', () => {
  ...
  it('flags missing media_type', () => {
    ...
    expect(issues.some((i) => i.node_key === 'm' && i.field === 'media_type')).toBe(true);
  });
  ...
  it('flags next_node_key pointing at a non-existent node', () => {
    ...
    expect(issues.some((i) => i.node_key === 'm' && i.field === 'next_node_key' && i.message.includes('ghost'))).toBe(true);
  });
...
describe('reachableFromEntry', () => {
  ...
  it('returns the entry alone when no edges lead out', () => {
    const set = reachableFromEntry('only', [{ node_key: 'only', node_type: 'handoff', config: {} }]);
