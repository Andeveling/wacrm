...
export interface FlowRunRow {
  ...
  user_id: string;
  /** NOT NULL — every active run must be tied to a contact. */
  contact_id: string;
  /** NOT NULL — every active run must be tied to a conversation. */
...
export interface DispatchInboundResult {
  ...
  flow_run_id?: string;
  /** For diagnostics. */
  outcome?: 'advanced' | 'started' | 'completed' | 'handed_off' | 'fallback_fired' | 'duplicate_inbound_ignored' | 'no_match';
}
