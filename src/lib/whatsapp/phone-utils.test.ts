...
import { isRecipientNotAllowedError, isValidE164, normalizePhone, phonesMatch, phoneVariants, sanitizePhoneForMeta } from './phone-utils';
...
describe('phoneVariants', () => {
  ...
  it('inserts a trunk 0 after each plausible country-code length', () => {
    ...
    const out = phoneVariants('37063949836');
    expect(out).toEqual(expect.arrayContaining(['37063949836', '307063949836', '370063949836']));
  });
...
describe('isRecipientNotAllowedError', () => {
  ...
  it('matches the human-readable English variants', () => {
    ...
    expect(isRecipientNotAllowedError('recipient not in the allowed list')).toBe(true);
  ...
  it('does not false-positive on unrelated Meta errors', () => {
    ...
    expect(isRecipientNotAllowedError('template name does not exist')).toBe(false);
