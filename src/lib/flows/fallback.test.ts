...
describe('resolveFallbackPolicy', () => {
  it('returns defaults for null / undefined / non-object', () => {
    ...
    expect(resolveFallbackPolicy('not-an-object')).toEqual(DEFAULT_FALLBACK_POLICY);
  ...
  it('preserves valid fields, defaults the rest', () => {
    expect(resolveFallbackPolicy({ max_reprompts: 5, on_exhaust: 'end' })).toEqual({
  ...
  it('rejects invalid on_unknown_reply values', () => {
    expect(resolveFallbackPolicy({ on_unknown_reply: 'nonsense' as unknown })).toEqual(DEFAULT_FALLBACK_POLICY);
  });

  it('rejects negative or NaN max_reprompts', () => {
    expect(resolveFallbackPolicy({ max_reprompts: -1 })).toEqual(DEFAULT_FALLBACK_POLICY);
    expect(resolveFallbackPolicy({ max_reprompts: Number.NaN })).toEqual(DEFAULT_FALLBACK_POLICY);
  });
  ...
  it('rejects non-positive on_timeout_hours', () => {
    expect(resolveFallbackPolicy({ on_timeout_hours: 0 })).toEqual(DEFAULT_FALLBACK_POLICY);
    expect(resolveFallbackPolicy({ on_timeout_hours: -5 })).toEqual(DEFAULT_FALLBACK_POLICY);
  });
});
...
describe('decideFallback', () => {
  ...
  it('reprompts up to max_reprompts', () => {
    ...
    expect(decideFallback({ policy: POLICY_REPROMPT_2_HANDOFF, reprompt_count: 1 })).toEqual({ type: 'reprompt' });
    expect(decideFallback({ policy: POLICY_REPROMPT_2_HANDOFF, reprompt_count: 2 })).toEqual({ type: 'reprompt' });
  });

  it('escalates to handoff once max_reprompts is exceeded', () => {
    // count=3 with max=2 → exhaust → handoff
    expect(decideFallback({ policy: POLICY_REPROMPT_2_HANDOFF, reprompt_count: 3 })).toEqual({ type: 'handoff' });
  });
