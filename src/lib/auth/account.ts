...
export async function getCurrentAccount(): Promise<AccountContext> {
  ...
  const { data, error } = await supabase.from('profiles').select('account_id, account_role').eq('user_id', user.id).maybeSingle();
  ...
  const { data: account, error: accountErr } = await supabase.from('accounts').select('id, name').eq('id', data.account_id).maybeSingle();
