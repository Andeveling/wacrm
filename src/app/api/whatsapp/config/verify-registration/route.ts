...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json({
      ...
      message: "Stored access token can't be decrypted — likely ENCRYPTION_KEY changed. Re-enter the token to repair.",
    });
  }
  ...
  const live = checks.phone_metadata_ok && (checks.waba_subscribed_to_app ?? false) && checks.locally_marked_registered;
