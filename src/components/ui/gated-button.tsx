...
export function GatedButton({ canAct = true, gateReason, title, disabled, className, children, ...rest }: GatedButtonProps) {
  const effectivelyDisabled = disabled || !canAct;
  const tooltip = !canAct && gateReason ? `Read-only — your role can't ${gateReason}` : title;
