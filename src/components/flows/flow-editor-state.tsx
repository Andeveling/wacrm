...
import { type ValidationIssue, validateFlowForActivation } from '@/lib/flows/validate';
import { type BuilderNode, NODE_META, type NodeType, slugify } from './shared';
...
export function applyNodePositions(nodes: BuilderNode[], positions: Record<string, { x: number; y: number }>): BuilderNode[] {
  return nodes.map((n) => {
  ...
  const setStatus = useCallback(
    async (next: BuilderState['status']) => {
      ...
      try {
        ...
        toast.success(next === 'active' ? t('statusActivated') : next === 'archived' ? t('statusArchived') : t('statusDraft'));
      } catch (err) {
    ...
    (type: NodeType): string => {
      ...
      setState((s) => {
        ...
        return {
          ...
          entry_node_id: s.entry_node_id ?? (type === 'start' ? node_key : (s.entry_node_id ?? null)),
        };
      });
      return createdKey;
    },
