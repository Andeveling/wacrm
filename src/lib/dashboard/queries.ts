...
import { DOW_SHORT_MON_FIRST, daysAgoStart, lastNDayKeys, localDayKey, mondayIndex, startOfLocalDay } from './date-utils';
import type {
  ActivityItem,
...
export async function loadMetrics(db: DB): Promise<MetricsBundle> {
  ...
  const [openConvCur, newConvToday, newConvYesterday, newContactsToday, newContactsYesterday, openDeals, messagesToday, messagesYesterday] =
    await Promise.all([
      db
        .from('conversations')
      ...
      db.from('messages').select('id', { count: 'exact', head: true }).eq('sender_type', 'agent').gte('created_at', todayStart),
      db
        .from('messages')
...
export async function loadConversationsSeries(db: DB, rangeDays: number): Promise<ConversationsSeriesPoint[]> {
...
export async function loadActivity(db: DB, limit = 20): Promise<ActivityItem[]> {
  ...
  const [msgs, contacts, deals, broadcasts, autoLogs] = await Promise.all([
    ...
    db.from('deals').select('id, title, updated_at, stage:pipeline_stages(name)').order('updated_at', { ascending: false }).limit(10),
    db.from('broadcasts').select('id, name, status, total_recipients, created_at').order('created_at', { ascending: false }).limit(5),
    db
