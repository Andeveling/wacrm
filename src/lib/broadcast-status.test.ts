...
import { broadcastStatusConfig, getBroadcastStatus, getRecipientStatus, recipientStatusConfig } from './broadcast-status';
...
describe('getBroadcastStatus', () => {
  ...
  it('falls back to draft on an unknown status string', () => {
    expect(getBroadcastStatus('not-a-real-status')).toBe(broadcastStatusConfig.draft);
...
describe('getRecipientStatus', () => {
  it('returns the matching config for known statuses', () => {
    expect(getRecipientStatus('delivered')).toBe(recipientStatusConfig.delivered);
