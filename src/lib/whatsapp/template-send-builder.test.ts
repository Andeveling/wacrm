...
describe('buildSendComponents — body', () => {
  ...
  it('emits a body component when the template has variables', () => {
    const components = buildSendComponents(row({ body_text: 'Hi {{1}}, order {{2}} confirmed.' }), { body: ['John', 'ORD-42'] });
  ...
  it('throws when body has variables but caller supplied too few values', () => {
    expect(() => buildSendComponents(row({ body_text: 'Hi {{1}} {{2}}' }), { body: ['just one'] })).toThrow(/2 variable\(s\) but only 1/);
  });

  it('trims extra body values silently (legacy callers may overshoot)', () => {
    const components = buildSendComponents(row({ body_text: 'Hi {{1}}' }), { body: ['John', 'extra', 'extra2'] });
    expect(components).toEqual([{ type: 'body', parameters: [{ type: 'text', text: 'John' }] }]);
  });
});

describe('buildSendComponents — header', () => {
  it('skips static TEXT headers (template carries them)', () => {
    expect(buildSendComponents(row({ header_type: 'text', header_content: 'Order Confirmation' }))).toEqual([]);
  });

  it('emits a TEXT header component when {{1}} is present', () => {
    const components = buildSendComponents(row({ header_type: 'text', header_content: 'Hello {{1}}' }), { headerText: 'Sara' });
    expect(components).toEqual([{ type: 'header', parameters: [{ type: 'text', text: 'Sara' }] }]);
  });

  it('throws when TEXT header has {{1}} but no value was supplied', () => {
    expect(() => buildSendComponents(row({ header_type: 'text', header_content: 'Hello {{1}}' }))).toThrow(
  ...
  it('auto-includes IMAGE header from the stored sample URL', () => {
    ...
    expect(components).toEqual([
      {
        type: 'header',
        parameters: [{ type: 'image', image: { link: 'https://example.com/sample.jpg' } }],
      },
    ]);
  });
  ...
  it('prefers caller override URL over template sample', () => {
    ...
    expect(components[0]).toEqual({
      type: 'header',
      parameters: [{ type: 'video', video: { link: 'https://example.com/custom.mp4' } }],
    });
  });
  ...
  it('uses an explicit headerMediaId override as the media id', () => {
    const components = buildSendComponents(row({ header_type: 'image', header_media_url: 'https://x.com/s.jpg' }), {
  ...
  it('throws on media header with no link OR id available', () => {
    expect(() => buildSendComponents(row({ header_type: 'image' }))).toThrow(/requires a media link or id/);
  });
});
...
describe('buildSendComponents — buttons', () => {
  ...
  it('overrides COPY_CODE code when caller supplies one', () => {
    ...
    expect((components[0] as { parameters: { coupon_code: string }[] }).parameters[0].coupon_code).toBe('PERSONAL_CODE');
  });

  it('skips PHONE_NUMBER buttons entirely (no send-time params allowed)', () => {
    const components = buildSendComponents(
      row({
        buttons: [{ type: 'PHONE_NUMBER', text: 'Call', phone_number: '+15551234567' }],
      })
    );
    expect(components).toEqual([]);
  });
