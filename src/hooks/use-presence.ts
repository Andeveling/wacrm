...
import { derivePresence, type PresenceRow, type PresenceStatus, type StoredPresence } from '@/lib/presence';
...
export function usePresence(enabled = true): UsePresenceResult {
  ...
  useEffect(() => {
    ...
    const applyRow = (row: { user_id: string; status: StoredPresence; last_seen_at: string }) => {
      setRows((prev) => {
      ...
      .on(
        ...
        (payload) => {
          if (payload.eventType === 'DELETE') {
            ...
            const deletedUserId = old.user_id;
            if (!deletedUserId) return;
            setRows((prev) => {
              if (!prev.has(deletedUserId)) return prev;
              const next = new Map(prev);
