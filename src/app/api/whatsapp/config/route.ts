...
import { type MetaPhoneInfo, registerPhoneNumber, subscribeWabaToApp, verifyPhoneNumber } from '@/lib/whatsapp/meta-api';
...
function supabaseAdmin() {
  if (!_adminClient) {
    _adminClient = createAdminClient(requireEnv('NEXT_PUBLIC_SUPABASE_URL'), requireEnv('SUPABASE_SERVICE_ROLE_KEY'));
  }
  return _adminClient;
}
...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      return NextResponse.json(
        {
          error: 'Failed to encrypt token. Check that ENCRYPTION_KEY is a valid 64-character hex string in your environment variables.',
        },
        { status: 500 }
      );
    }
...
export async function DELETE() {
  try {
    ...
    const { error: deleteError } = await supabase.from('whatsapp_config').delete().eq('account_id', accountId);
