...
describe('hashInviteToken', () => {
  ...
  it('matches a known SHA-256 hex digest', () => {
    ...
    expect(hashInviteToken('invite-token-abc')).toBe('51481b404112f61a4e1171ff116d52068c429737863181bef089df7cb607352f');
  });
});

describe('inviteUrl', () => {
  it('joins path correctly with no trailing slash', () => {
    expect(inviteUrl('abc', 'https://wacrm.example')).toBe('https://wacrm.example/join/abc');
  });
...
describe('inviteExpiresAt', () => {
  ...
  it('uses the default when expiresInDays is omitted', () => {
    ...
    const expected = new Date(now.getTime() + DEFAULT_INVITE_EXPIRY_DAYS * 24 * 60 * 60 * 1000);
  ...
  it('respects the max clamp', () => {
    ...
    const expected = new Date(now.getTime() + MAX_INVITE_EXPIRY_DAYS * 24 * 60 * 60 * 1000);
    expect(out.toISOString()).toBe(expected.toISOString());
  });
});
