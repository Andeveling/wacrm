...
describe('encryption', () => {
  ...
  describe('GCM authentication', () => {
    it('rejects ciphertext tampered after encryption', () => {
      ...
      const tamperedCtHex = (parseInt(ctHex.slice(0, 2), 16) ^ 0xff).toString(16).padStart(2, '0') + ctHex.slice(2);
    ...
    it('rejects a GCM IV of the wrong length', () => {
      ...
      expect(() => decrypt(`${shortIv}:${ctHex}:${tagHex}`)).toThrow(/GCM IV length/);
    });
    ...
    it('rejects a GCM auth tag of the wrong length', () => {
      ...
      expect(() => decrypt(`${ivHex}:${ctHex}:${shortTag}`)).toThrow(/auth-tag length/);
    });
  });
  ...
  describe('malformed input', () => {
    it('throws on a single-token blob (no colons)', () => {
      expect(() => decrypt('not-encrypted-at-all')).toThrow(/unrecognised format/);
    });
