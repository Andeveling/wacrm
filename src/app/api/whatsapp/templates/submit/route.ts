...
import { type TemplatePayload, validateTemplatePayload } from '@/lib/whatsapp/template-validators';
  ...
  try {
    ...
    if (payload.category === 'Authentication') {
      return NextResponse.json(
        {
          error: 'AUTHENTICATION templates are not yet supported here — create them in Meta WhatsApp Manager and use "Sync from Meta".',
        },
        { status: 400 }
      );
    }
    ...
    if (dryRun) {
      ...
      try {
        ...
        return NextResponse.json(
          {
            error: isRateLimit ? 'Meta rate limit hit (100 template creates per hour). Try again later.' : message,
          },
