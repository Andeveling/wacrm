...
import { engineSendInteractiveButtons, engineSendInteractiveList, engineSendMedia, engineSendText } from './meta-send';
import type {
 ...
 */
export function matchReplyId(node: { node_type: string; config: Record<string, unknown> }, reply_id: string): string | null {
...
export function isSuspending(node_type: string): boolean {
  return node_type === 'send_buttons' || node_type === 'send_list' || node_type === 'collect_input';
}
...
async function loadActiveRunForContact(db: AdminClient, accountId: string, contactId: string): Promise<FlowRunRow | null> {
 ...
 */
async function isDuplicateInbound(db: AdminClient, accountId: string, contactId: string, metaMessageId: string): Promise<boolean> {
  // Fetch ALL run ids for this contact in this account (active +
  // historical). Bounded by how many flows the customer has been
  // through — small.
  const { data: runs } = await db.from('flow_runs').select('id').eq('account_id', accountId).eq('contact_id', contactId);
...
async function findEntryFlow(db: AdminClient, accountId: string, message: ParsedInbound, isFirstInbound: boolean): Promise<FlowRow | null> {
...
async function sendButtonsAndSuspend(
  ...
  const { data: msg } = await db.from('messages').select('id').eq('message_id', whatsapp_message_id).maybeSingle();
  await db
    .from('flow_runs')
    .update({
...
async function sendListAndSuspend(db: AdminClient, run: FlowRunRow, node: FlowNodeRow): Promise<{ outcome: 'advanced'; node_key: string }> {
  const cfg = node.config as unknown as SendListNodeConfig;
...
async function executeHandoff(db: AdminClient, run: FlowRunRow, node: FlowNodeRow): Promise<void> {
 ...
 */
async function evaluateConditionNode(db: AdminClient, run: FlowRunRow, cfg: ConditionNodeConfig): Promise<boolean> {
  ...
  if (cfg.subject === 'var') {
    ...
    const { data } = await db.from('contacts').select(cfg.subject_key).eq('id', run.contact_id).maybeSingle();
 ...
 */
async function advanceCurrentNodeKey(db: AdminClient, runId: string, expectedOldKey: string | null, newKey: string): Promise<boolean> {
...
async function handleReplyForActiveRun(
  ...
  if (action.type === 'handoff') {
    if (run.conversation_id) {
      await db.from('conversations').update({ status: 'pending', updated_at: new Date().toISOString() }).eq('id', run.conversation_id);
    }
