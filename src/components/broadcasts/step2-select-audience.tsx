...
import { ArrowLeft, ArrowRight, Filter, Loader2, Tags, Upload, Users, X } from 'lucide-react';
...
export function Step2SelectAudience({ audience, onUpdate, onNext, onBack }: Step2Props) {
  ...
  const fetchEstimatedCount = useCallback(async () => {
    ...
    try {
      ...
      if (audience.type === 'all') {
        ...
        const phones = [...new Set(audience.csvContacts.map((contact) => normalizePhone(contact.phone)).filter(Boolean))];
        const { data } = await supabase
          .from('contacts')
          .select('phone_normalized, archived_at')
          .in('phone_normalized', phones)
          .not('archived_at', 'is', null);
        const archivedPhones = new Set((data ?? []).map((contact) => contact.phone_normalized));
      ...
      const { data: activeContacts } = await supabase.from('contacts').select('id').is('archived_at', null);
      const activeIds = new Set((activeContacts ?? []).map((contact) => contact.id));
      const effective = baseIds ?? activeIds;
      setEstimatedCount([...effective].filter((id) => activeIds.has(id) && !excludeSet?.has(id)).length);
    } finally {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-semibold text-foreground text-lg">{t('selectAudience.title')}</h2>
        <p className="mt-1 text-muted-foreground text-sm">{t('selectAudience.subtitle')}</p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div>
                ...
                <p className="mt-0.5 text-muted-foreground text-xs">{option.description}</p>
              </div>
            </button>
          );
        })}
      ...
      {audience.type === 'tags' && (
        <div className="rounded-xl border border-border bg-card/50 p-4">
          <p className="mb-3 font-medium text-foreground text-sm">{t('selectAudience.selectTags')}</p>
          {loadingTags ? (
            <Loader2 className="h-5 w-5 animate-spin text-primary" />
          ) : tags.length === 0 ? (
            <p className="text-muted-foreground text-xs">{t('selectAudience.noTagsFound')}</p>
          ) : (
              ...
              {tags.map((tag) => {
                ...
                return (
                  <button
                    ...
                    className={`inline-flex items-center rounded-full border px-3 py-1 font-medium text-xs transition-all ${
                      isSelected
      ...
      {audience.type === 'custom_field' && (
        <div className="space-y-3 rounded-xl border border-border bg-card/50 p-4">
          <p className="font-medium text-foreground text-sm">{t('selectAudience.method.customField')}</p>
          {loadingFields ? (
            <Loader2 className="h-5 w-5 animate-spin text-primary" />
          ) : customFields.length === 0 ? (
            <p className="text-muted-foreground text-xs">{t('selectAudience.errorLoadFields')}</p>
          ) : (
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-[minmax(0,1fr)_140px_minmax(0,1fr)]">
              <select
                value={audience.customField?.fieldId ?? ''}
                onChange={(e) => updateCustomField({ fieldId: e.target.value })}
                className="h-9 rounded-lg border border-border bg-muted px-2.5 text-foreground text-sm outline-none focus:border-primary focus:ring-1 focus:ring-primary"
              >
              ...
              <input
                ...
                className="h-9 rounded-lg border border-border bg-muted px-2.5 text-foreground text-sm outline-none placeholder:text-muted-foreground focus:border-primary focus:ring-1 focus:ring-primary"
              />
            </div>
          )}
        </div>
      ...
      <div className="rounded-xl border border-border bg-card/50 p-4">
        <div className="mb-3 flex items-center gap-2">
          ...
          <p className="font-medium text-foreground text-sm">{t('selectAudience.excludeTags')}</p>
        </div>
        {tags.length === 0 ? (
          <p className="text-muted-foreground text-xs">{t('selectAudience.noTagsFound')}</p>
        ) : (
      ...
      {/* Audience Summary */}
      <div className="rounded-xl border border-border bg-card/50 p-4">
        <p className="mb-2 font-medium text-foreground text-sm">Audience Summary</p>
        {loadingCount ? (
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-foreground text-sm">{estimatedCount.toLocaleString()}</span>
            <span className="text-muted-foreground text-xs">estimated recipients</span>
          </div>
        ) : (
          <p className="text-muted-foreground text-xs">Select an audience type to see the estimate.</p>
        )}
      </div>
