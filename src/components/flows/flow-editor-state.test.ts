...
import { applyNodePositions, defaultConfigFor, uniqueNodeKey } from './flow-editor-state';
...
describe('uniqueNodeKey', () => {
  it("returns the base key when it isn't taken", () => {
    ...
    expect(uniqueNodeKey('menu', [{ node_key: 'other', node_type: 'end', config: {} }])).toBe('menu');
  });

  it('appends _2 when the base is taken', () => {
    expect(uniqueNodeKey('menu', [{ node_key: 'menu', node_type: 'end', config: {} }])).toBe('menu_2');
  });
