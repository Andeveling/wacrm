...
export function resolveFallbackPolicy(raw: unknown): FlowFallbackPolicy {
  ...
  return {
    on_unknown_reply:
      r.on_unknown_reply === 'handoff' || r.on_unknown_reply === 'ignore' || r.on_unknown_reply === 'reprompt'
        ? r.on_unknown_reply
        : DEFAULT_FALLBACK_POLICY.on_unknown_reply,
    max_reprompts:
      typeof r.max_reprompts === 'number' && r.max_reprompts >= 0 ? Math.floor(r.max_reprompts) : DEFAULT_FALLBACK_POLICY.max_reprompts,
    on_timeout_hours:
      typeof r.on_timeout_hours === 'number' && r.on_timeout_hours > 0 ? r.on_timeout_hours : DEFAULT_FALLBACK_POLICY.on_timeout_hours,
    on_exhaust: r.on_exhaust === 'handoff' || r.on_exhaust === 'end' ? r.on_exhaust : DEFAULT_FALLBACK_POLICY.on_exhaust,
  };
}
