...
function makeConversation(contact: Partial<Conversation['contact']> | null): Conversation {
  return {
    id: 'c1',
    user_id: 'u1',
    contact_id: 'ct1',
...
describe('matchesContactFilters', () => {
  ...
  it('uses OR logic across tags', () => {
    ...
    expect(matchesContactFilters(conv, { tagIds: ['t2', 't9'], company: null })).toBe(true);
    expect(matchesContactFilters(conv, { tagIds: ['t9'], company: null })).toBe(false);
  });

  it('excludes conversations whose contact has no tags when a tag filter is active', () => {
    const conv = makeConversation({ tags: [] });
    expect(matchesContactFilters(conv, { tagIds: ['t1'], company: null })).toBe(false);
    expect(
      matchesContactFilters(makeConversation(null), {
        tagIds: ['t1'],
        company: null,
  ...
  it('matches company exactly, trimming whitespace', () => {
    ...
    expect(matchesContactFilters(conv, { tagIds: [], company: 'Acme' })).toBe(true);
    expect(matchesContactFilters(conv, { tagIds: [], company: 'Other' })).toBe(false);
  });

  it('requires both tag and company to match when both are set (AND across facets)', () => {
    const conv = makeConversation({ company: 'Acme', tags: [tag('t1')] });
    expect(matchesContactFilters(conv, { tagIds: ['t1'], company: 'Acme' })).toBe(true);
    expect(matchesContactFilters(conv, { tagIds: ['t1'], company: 'Other' })).toBe(false);
    expect(matchesContactFilters(conv, { tagIds: ['tX'], company: 'Acme' })).toBe(false);
  });
});
...
describe('normalizeConversation', () => {
  it('flattens embedded contact_tags into contact.tags', () => {
    ...
    expect((normalized.contact as unknown as Record<string, unknown>).contact_tags).toBeUndefined();
  });
