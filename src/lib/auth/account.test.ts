...
function makeClient(opts: { user: { id: string } | null; userErr?: unknown; byTable: Record<string, { data: unknown; error: unknown }> }) {
  ...
  const from = (table: string) => {
    ...
    const builder = {
      ...
      maybeSingle() {
        return Promise.resolve(opts.byTable[table] ?? { data: null, error: null });
      },
    };
    return builder;
  };
...
const { getCurrentAccount, UnauthorizedError, ForbiddenError } = await import('./account');
...
describe('getCurrentAccount', () => {
  ...
  it('rejects a profile not linked to an account', async () => {
    ...
    createClient.mockReturnValue(client);
    await expect(getCurrentAccount()).rejects.toThrow('Profile is not linked to an account');
  });
