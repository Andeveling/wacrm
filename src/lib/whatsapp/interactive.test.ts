...
describe('validateInteractivePayload — buttons', () => {
  ...
  it('rejects empty button id / title', () => {
    expect(validateInteractivePayload({ ...validButtons, buttons: [{ id: '', title: 'A' }] }).ok).toBe(false);
    expect(validateInteractivePayload({ ...validButtons, buttons: [{ id: 'a', title: '' }] }).ok).toBe(false);
  });
});
...
describe('validateInteractivePayload — list', () => {
  ...
  it('requires a button label within 20 chars', () => {
    ...
    expect(validateInteractivePayload({ ...validList, button_label: 'x'.repeat(21) }).ok).toBe(false);
  });
