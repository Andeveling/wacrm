 ...
 */
export function normalizeUsage(raw: { prompt?: unknown; completion?: unknown; total?: unknown }): AiUsage | null {
...
export async function providerHttpError(provider: string, res: Response): Promise<AiError> {
  ...
  const { status } = res;
  const code = status === 401 || status === 403 ? 'invalid_key' : status === 429 ? 'rate_limited' : 'provider_error';
  const base =
    code === 'invalid_key'
