...
vi.mock('./admin-client', () => {
  ...
  function resolve(ops: { table: string; type: string; payload?: unknown; filters: [string, string, unknown][] }) {
  ...
  function builder(table: string) {
    ...
    const setType = (type: string, payload?: unknown) => {
      ops.type = type;
      if (payload !== undefined) ops.payload = payload;
      return b;
    };
    const b: Record<string, unknown> = {
      select: () => b,
      insert: (p: unknown) => setType('insert', p),
      update: (p: unknown) => setType('update', p),
      delete: () => setType('delete'),
      upsert: (p: unknown) => setType('upsert', p),
      ...
      then: (onF: (v: unknown) => unknown, onR?: (e: unknown) => unknown) => Promise.resolve(resolve(ops)).then(onF, onR),
    };
    return b;
  }
...
describe('update_contact_field — custom fields', () => {
  ...
  it('interpolates {{ vars.* }} into the custom value', async () => {
    ...
    expect(h.state.upsertCalls).toHaveLength(1);
    expect((h.state.upsertCalls[0].payload as { value: string }).value).toBe('WhatsApp Ad');
  });
...
describe('triggerMatches — interactive_reply', () => {
  ...
  it('matches when the tapped id is in reply_ids (exact)', () => {
    expect(triggerMatches(automation(['yes', 'no']), { interactive_reply_id: 'yes' })).toBe(true);
  });

  it('does not match a different id', () => {
    expect(triggerMatches(automation(['yes']), { interactive_reply_id: 'maybe' })).toBe(false);
  });

  it('does not match on a substring (exact only)', () => {
    expect(triggerMatches(automation(['yes']), { interactive_reply_id: 'yes_please' })).toBe(false);
  });
