...
export async function POST(request: Request) {
  try {
    ...
    if (!apiKeyPlain) {
      const { data: existing } = await supabase.from('ai_configs').select('api_key').eq('account_id', accountId).maybeSingle();
      if (!existing?.api_key) {
      ...
      try {
        ...
        return NextResponse.json({ error: 'Stored API key could not be decrypted — re-enter your key.' }, { status: 400 });
      }
    }
