-- Public API calls run with the service-role key after API-key scope checks.
-- The optional account argument keeps that trusted path account-scoped while
-- preserving the existing authenticated dashboard RPC calls.
